...
const approveByCode = async (req, res, next) => {
    ...
    try {
        ...
        // ✅ Check if the code has already been used for this edit request
        const codeUsedForRequest = await CodeUsers.findOne({
            where: { codeId: existingCode.id, userId: id, editRequestId: id },
            transaction,
        });

        if (codeUsedForRequest) {
            await transaction.rollback();
            return next(new AppError("This code has already been used for this edit request", 400));
        }
        ...
        // ✅ Store who used the code for this specific request
        await CodeUsers.create(
            {
                codeId: existingCode.id,
                userId: record.userId,
                editRequestId: id, // 👈 Track code usage per edit request
            },
            { transaction }
        );
